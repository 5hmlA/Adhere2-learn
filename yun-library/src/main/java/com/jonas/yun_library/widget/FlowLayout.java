...
public class FlowLayout extends ViewGroup implements CompoundButton.OnCheckedChangeListener {
    ...
    private CompoundButton lastSelected;
...
//                    if (!(mMaxLinesCount != Integer.MAX_VALUE && count < mMaxLinesCount && !mNeedExpend)) {
    ...
    // 初始化条件布局
    private CheckBox newCheckBox(String name, int strselector, int bgselector) {
    ...
    public void onCheckedChanged(CompoundButton compoundButton, boolean b) {
        if (mSingleSelecte && b) {
            ...
            lastSelected = compoundButton;
        } else {
            lastSelected = null;
        }
        if (mListener != null) {
            mListener.onItemSelected(compoundButton, (Integer) compoundButton.getTag());
        }
    }
